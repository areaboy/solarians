...
$cctx = $db->prepare('select * from users_solar where userid =:userid');
...
$res= $db->prepare("SELECT count(*) as totalcount FROM posts_solar WHERE userid=:userid");
...
$result = $db->prepare("SELECT * FROM posts_solar WHERE userid=:userid order by id desc limit :row1, :rowpage");
