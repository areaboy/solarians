...
$resu = $db->prepare('SELECT * FROM post_like_solar WHERE  postid=:postid and userid=:userid');
...
$statement = $db->prepare('INSERT INTO post_like_solar
...
$statement2 = $db->prepare('INSERT INTO notification_solar
...
$cct = $db->prepare('select * from posts_solar where id=:id');
$cct->execute(array(':id' =>$post_id));
$rct_row = $cct->fetch();
$totallikes = $rct_row['total_like'];
$total_l = $totallikes + 1;

$update2= $db->prepare('UPDATE posts_solar set total_like =:total_like where id=:id');
...
//$resultp = $db->prepare('SELECT COUNT(*) AS cnt FROM post_like_solar WHERE postid=:postid');
$resultp = $db->prepare('SELECT COUNT(*) AS cnt FROM post_like_solar WHERE  postid=:postid');
