...
if($lat==''){
echo "<script>alert('Ensure there is an Internet Connection. There is an issue with Company Address Google Geo-Codings. Pls Try Again');</script>";
echo "<div style='background:red;padding:8px;color:white;border:none;' id='alerts_reg'>Ensure there is an Internet Connection. There is an issue with Company Address Google Geo-Codings. Pls Try Again</div>";
exit();
...
$result_verified = $db->prepare('select * from company_solar where email=:email');
...
$statement = $db->prepare('INSERT INTO company_solar
(email,company_name,company_desc,created_time,timing,address,photo,lat,lng,country_name,country)
                          values
(:email,:company_name,:company_desc,:created_time,:timing,:address,:photo,:lat,:lng,:country_name,:country)');
